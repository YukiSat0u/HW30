...
void quicksort(ThreadPool& pool, int* array, long left, long right) {
	if (left >= right) {
		return;
	}
	...
	if (make_thread && (right_bound - left > 10000)) {
		if ((right_bound - left_bound) > 100000) {  // Условие для выполнения подзадачи
			...
			f.wait();  // Ожидаем завершение подзадачи
		}
		else {
			quicksort(pool, array, left, right_bound);
			quicksort(pool, array, left_bound, right);
